...
% Images in the same directory as the .tex file
\graphicspath{{./}}
...
\textbf{Code and Data Availability.} To guarantee full reproducibility and technical scrutiny, the complete source code, the modular orchestration engine, and the interactive \emph{Power~BI} templates of this research are openly published at the official repository: \url{https://github.com/faritreascodev/fintech-etl-pipeline}.
% ========= 2. Related Work =========
...
Fig.~\ref{fig:architecture} summarizes the flow. In \textbf{ingestion}, data is captured in spreadsheets (Excel~/.xlsx or Google Sheets) functioning as a predictable source. In \textbf{processing}, a modular Python script runs the idempotent ETL (\texttt{extract.py}, \texttt{transform.py}, \texttt{load.py}) and generates artifacts with metadata and hashes. In \textbf{presentation}, Power~BI imports the artifacts \emph{as~is} and \texttt{visualize.py} generates the scientific figures for the paper.
...
\node[box, fill=gray!10] (sheets) {Excel / Google Sheets\\(.xlsx or CSV)};
\node[box, right=of sheets, fill=blue!6] (etl) {Python \& Pandas\\Modular ETL\\\footnotesize extract $\to$ transform $\to$ load};
\node[box, below=of etl, fill=orange!10] (cs) {Parquet Artifacts\\\footnotesize daily\_portfolio, exposure\_sector,\\exposure\_ticker, positions};
\node[box, right=of etl, fill=green!8] (pbi) {Power BI\\Dashboard\\\footnotesize 3 thematic pages};
\node[box, above=of etl, fill=red!8] (dq) {Data Quality\\\footnotesize Types, NAs/$\pm\infty$,\\Densification, Rebase};
\node[box, below=of cs, fill=violet!8] (viz) {\texttt{visualize.py}\\\footnotesize 8 PNG figures (DPI=300)};
\draw[-{Latex[length=3mm]}] (sheets) -- node[above]{\footnotesize ingestion} (etl);
\draw[-{Latex[length=3mm]}] (etl) -- node[above]{\footnotesize load} (pbi);
\draw[-{Latex[length=3mm]}] (etl) -- (cs);
\draw[-{Latex[length=3mm]}] (dq) -- (etl);
\draw[-{Latex[length=3mm]}] (cs) -- (viz);
\end{tikzpicture}}
\caption{Pipeline architecture: ingestion, modular ETL processing, Parquet storage, and outputs (BI dashboard and scientific figures).}
...
\begin{table}[!t]
\centering
\caption{ETL vs.\ Oracle Comparison (computed from Parquet artifacts)}
\label{tab:oracle}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}llll@{}}
\toprule
...
\midrule
Mean daily return (\%) & 0.0925 & 0.0925 & $<$1 \\
21d Volatility annualized (\%) & 15.07 & 15.07 & $<$1 \\
End-of-period cumulative & 1.6749 & 1.6749 & $<$1 \\
Mean index gap (\%) & \multicolumn{2}{l}{N/A (index data pending)} & --- \\
\bottomrule
\end{tabular}}
\vspace{-2mm}
\end{table}
...
\textbf{Internal.} Daily returns are computed by discounting new capital flows (purchases and sales) from the portfolio value, thereby isolating price-driven P\&L. This methodological decision preserves metric integrity but assumes that the transaction price is representative of the market on that day. \\
