...
% Imágenes en el mismo directorio que el .tex
\graphicspath{{./}}
...
\author{%
\IEEEauthorblockN{Farit Alexander Reasco Torres}
\IEEEauthorblockA{Tecnología Superior en Desarrollo de Software\\Pontificia Universidad Católica del Ecuador Sede Esmeraldas\\ \texttt{fareasco@pucese.edu.ec}}
\and
\IEEEauthorblockN{Andrés Ricardo Guanoluisa Plasencia}
\IEEEauthorblockA{Pontificia Universidad Católica del Ecuador Sede Esmeraldas\\ \texttt{arguanoluisa@pucese.edu.ec}}}
...
\textbf{Disponibilidad de Código y Datos.} Para certificar la reproducibilidad absoluta y el escrutinio técnico, el código fuente completo, el motor orquestador modular y las plantillas interactivas \emph{Power~BI} de esta investigación están disponibles de manera abierta en el repositorio oficial: \url{https://github.com/faritreascodev/fintech-etl-pipeline}.
...
La Fig.~\ref{fig:arquitectura} resume el flujo. En \textbf{ingesta}, los datos se capturan en hojas de cálculo (Excel~/.xlsx o Google Sheets) que funcionan como fuente predecible. En \textbf{procesamiento}, un script modular ejecuta el ETL idempotente (\texttt{extract.py}, \texttt{transform.py}, \texttt{load.py}) y genera artefactos con metadatos y huellas. En \textbf{presentación}, Power~BI importa los artefactos \emph{as~is} y el módulo \texttt{visualize.py} genera las figuras científicas del paper.
...
\node[box, fill=gray!10] (sheets) {Excel / Google Sheets\\(.xlsx o CSV)};
\node[box, right=of sheets, fill=blue!6] (etl) {Python \& Pandas\\ETL Modular\\\footnotesize extract $\to$ transform $\to$ load};
\node[box, below=of etl, fill=orange!10] (cs) {Artefactos Parquet\\\footnotesize cartera\_diaria, exposición\_sector,\\exposición\_ticker, posiciones};
\node[box, right=of etl, fill=green!8] (pbi) {Power BI\\Dashboard\\\footnotesize 3 páginas temáticas};
\node[box, above=of etl, fill=red!8] (dq) {Calidad de datos\\\footnotesize tipos, NAs/$\pm\infty$,\\densificación, rebase};
\node[box, below=of cs, fill=violet!8] (viz) {\texttt{visualize.py}\\\footnotesize 8 figuras PNG (DPI=300)};
\draw[-{Latex[length=3mm]}] (sheets) -- node[above]{\footnotesize ingesta} (etl);
\draw[-{Latex[length=3mm]}] (etl) -- node[above]{\footnotesize carga} (pbi);
\draw[-{Latex[length=3mm]}] (etl) -- (cs);
\draw[-{Latex[length=3mm]}] (dq) -- (etl);
\draw[-{Latex[length=3mm]}] (cs) -- (viz);
\end{tikzpicture}}
\caption{Arquitectura del pipeline: ingesta, procesamiento ETL, almacenamiento Parquet y salidas (BI y figuras científicas).}
...
\input{tab_latencia.tex}

\subsection{Escalabilidad (10$\times$--50$\times$)}
\input{tab_escala.tex}
...
\begin{table}[!t]
\centering
\caption{Comparación ETL vs.\ oráculo (calculado sobre artefactos Parquet)}
\label{tab:oraculo}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}llll@{}}
\toprule
...
\midrule
Retorno medio diario (\%) & 0{,}0925 & 0{,}0925 & $<$1 \\
Volatilidad 21d anualiz.\ (\%) & 15{,}07 & 15{,}07 & $<$1 \\
Acumulado fin de periodo & 1{,}6749 & 1{,}6749 & $<$1 \\
Brecha media vs.\ índice (\%) & \multicolumn{2}{l}{N/A (datos índice pendientes)} & --- \\
\bottomrule
\end{tabular}}
\vspace{-2mm}
\end{table}
...
\textbf{Interna.} El retorno diario se calcula descontando flujos de capital nuevo (compras y ventas) del valor de cartera del período, aislando así el P\&L de movimiento de precios. Esta decisión metodológica preserva la integridad de las métricas pero asume que el precio de transacción es representativo del mercado en ese día. \\
