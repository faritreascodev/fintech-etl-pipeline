...
% ==== Metadatos ====
\title{Arquitectura ligera y reproducible para análisis de portafolios financieros: diseño, implementación y evaluación con Google Sheets, Python y Power~BI}
...
% ========= Abstract =========
\begin{abstract}
Se presenta una arquitectura de analítica financiera orientada a portafolios que prioriza \emph{simplicidad operativa, reproducibilidad y bajo costo}. El flujo completo se apoya en tres pilares: (i) \emph{Google Sheets} como repositorio colaborativo publicado como CSV; (ii) un proceso ETL en \emph{Python/Pandas} (Google Colab o entorno local) que normaliza datos, valida reglas de calidad, calcula indicadores de desempeño y riesgo y genera artefactos listos para consumo; y (iii) \emph{Power~BI} como capa de visualización interactiva. El sistema evita dependencias de credenciales complejas y se diseña para ser idempotente: una misma entrada produce los mismos resultados, acompañados de huellas (\emph{hashes}) y metadatos que facilitan su trazabilidad. Se describe cómo se gestiona la densificación del calendario de precios, cómo se comparan portafolio e índice mediante \emph{rebase} y cómo se construyen visualizaciones de alto valor interpretativo (acumulados, distribución de retornos, volatilidad móvil, exposición sectorial, contribución de \emph{tickers}, \emph{drawdown} y estacionalidad de retornos). El caso de estudio, basado en hojas y CSV reales, confirma tiempos de refresco adecuados para el aula y el laboratorio, a la vez que mantiene criterios de calidad y transparencia alineados con buenas prácticas de inteligencia de negocios. Finalmente, se discuten limitaciones y una ruta de evolución hacia servicios, autenticación y orquestación.
\end{abstract}
...
\node[box, below=of etl, fill=orange!10] (cs) {Artefactos CSV\\\footnotesize cartera\_diaria, exposición,\\estadísticos, benchmark};
...
La carpeta de salida incluye archivos temáticos (cartera, exposición, estadísticos, benchmark) y un subdirectorio con figuras reproducibles en PNG y PDF. Cada archivo puede acompañarse de un \texttt{.meta.json} con fecha de ejecución, parámetros, \emph{hash} y versión del script. Esta organización permite rehacer el tablero en minutos y auditar cambios.
...
\begin{itemize}
    ...
    \item \textbf{Presentación:} PBI continúa consumiendo CSV/Parquet; si hay lago de datos, se habilita \emph{Direct Lake}.
\end{itemize}
